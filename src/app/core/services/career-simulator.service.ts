...
export class CareerSimulatorService {
  ...
  private sessionSubject = new BehaviorSubject<SimulatorSessionState | null>(
    null,
  );
    ...
    return this.http.get<any[]>(`${environment.apiUrl}/career-simulators`).pipe(
      map((sims) =>
        sims.map((sim) => ({
          ...
          areaClass: sim.colorToken
            ? sim.colorToken
            : this.inferAreaClass(sim.steamArea || sim.steamAreaName),
          areaEmoji:
            sim.icon || this.inferAreaEmoji(sim.steamArea || sim.steamAreaName),
  ...
  public getSimulator(slug: string): Observable<CareerSimulatorData> {
    return this.http
      .get<any>(`${environment.apiUrl}/career-simulators/${slug}`)
      .pipe(
        map((sim) => ({
          careerId: sim.slug || sim.id,
          careerName: sim.careerName,
  ...
  public startSession(careerSlug: string): void {
    ...
    this.getSimulator(careerSlug).subscribe({
      ...
      error: (err) => {
        console.error(
          `No se pudo cargar la sesión del simulador ${careerSlug} desde la API:`,
          err,
        );
        this.sessionSubject.next({
          ...this.initialState,
          isLoadingSimulator: false,
          loadError:
            'No pudimos cargar esta simulación. Revisa tu conexión e inténtalo de nuevo.',
        });
      },
    });
  }
  ...
  public detectLinearPattern(): boolean {
    ...
    const applicableDecisions = state.userDecisions.filter(
      (d) =>
        (d.stepType === 'DATA_ANALYSIS' ||
          d.stepType === 'TRADEOFF_DECISION') &&
        d.selectedOptionId,
    );
    ...
    const optionIndices = applicableDecisions
      .map((decision) => {
        const stepData = state.currentCareerData?.steps.find(
          (s) => s.id === decision.stepId,
        );
        if (!stepData || !stepData.options) return -1;
        return stepData.options.findIndex(
  ...
  public computeAffinityResult(): Observable<SimulatorFeedbackResponse> {
    ...
    if (!state?.currentCareerData) {
      return throwError(
        () => new Error('No hay una sesión activa de simulación.'),
      );
    }

    const career = state.currentCareerData;
    this.sessionSubject.next({ ...state, isLoadingFeedback: true });
    const payload = {
    ...
    return this.profileService.submitSimulatorSession(payload).pipe(
      ...
      tap((feedback) => {
        this.persistSimulatorResult(
          career.careerId,
          career.steamAreaName,
          feedback,
        );
        this.sessionSubject.next({
          ...state,
      ...
      catchError((err) => {
        console.warn(
          'Simulador: API no disponible, usando algoritmo local.',
          err,
        );
  ...
  private computeAffinityLocally(
    state: SimulatorSessionState,
  ): Observable<SimulatorFeedbackResponse> {
    const career = state.currentCareerData!;
    try {
  ...
  private runAffinityAlgorithm(
    ...
    const steamAccum: Record<string, number> = {
      ciencia: 0,
      tecnologia: 0,
      ingenieria: 0,
      artes: 0,
    ...
    for (const decision of decisions) {
      ...
      if (option.steamTraitWeight) {
        for (const [axis, weight] of Object.entries(
          option.steamTraitWeight as Record<string, number>,
        )) {
    ...
    const primaryScore =
      scoredDecisions > 0 ? (steamScores[primaryAxis] ?? 60) : 60;
    ...
    const confidenceLevel: 'high' | 'medium' | 'low' =
      state.biasFlags.too_fast && state.biasFlags.linear_pattern_detected
        ? 'low'
        : state.biasFlags.too_fast || state.biasFlags.linear_pattern_detected
          ? 'medium'
          : 'high';
    ...
    const sortedAxes = Object.entries(steamScores).sort(
      ([, a], [, b]) => b - a,
    );
  ...
  private persistSimulatorResult(
    careerSlug: string,
    steamAreaName: string,
    result: SimulatorFeedbackResponse,
  ): void {
    const userId = this.authService.getCurrentUser()?.id;
    if (!userId) return;
    const key = `simulator_results_${userId}`;
    try {
      const existing: SimulatorAffinityResult[] = JSON.parse(
   ...
   */
  public getMyResults(): Observable<
    Array<{
      careerSlug: string;
      axis: string;
      affinity: number;
