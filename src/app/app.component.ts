...
export class AppComponent implements OnInit {
  ...
  private offlineSyncService = inject(OfflineSyncService); // Inicializa la outbox offline
    ...
    this.router.events
      ...
      .subscribe((event) => {
        ...
        const isChunkLoadFailure =
          /Failed to fetch dynamically imported module|Loading chunk|ChunkLoadError|error loading dynamically imported module/i.test(
            message,
          );
        const alreadyRetried =
          sessionStorage.getItem('chunk-load-retry') === '1';
